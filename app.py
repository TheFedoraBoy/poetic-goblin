...
def set_security_headers(response):
    ...
    # Build dynamic connect-src for Sentry and S3
...
# ─── Health Check (Railway/load balancer monitoring) ──────────────────────────

@app.route('/health')
def health_check():
    """Lightweight health check for Railway deploy monitoring."""
    try:
        db = get_db()
...
ELYSAL_LOCATIONS = [
    # Cities & Settlements
    "Hypolta, the Oldest City", "Clarity, Capital of Lambridge",
    "Oradova, the University City", "Palasor, the Imperial Capital",
    "Rin Nohara, City of Ships", "Boulavar, the Industrial Heart",
    "Foebrier, the Debrear Stronghold", "Imbraxione, City of Dragonborn",
    "Capera Cuza, the Contested City", "Catcher's Rim, Isle of Lipse",
    "Havenaugh, the Pervin Hills", "Voskraad, Raxonian Port",
    "Korallis, Veran Harbor", "Sheah Suraya, the First Colony",
    "Mural Cere, the Holy City",
    # Kingdoms & Nations
    "The Kingdom of Raxone", "The Gechann Empire",
    "The Kingdom of Lambridge", "The Kingdom of Hyacinth",
    "The Republic of Epervinay", "The Kingdom of Vera",
    "The Republic of Karagon", "Deberania, the Forest Kingdom",
    "The Lirzan Kingdom", "The Unified Perut",
    # Geographic Features & Landmarks
    "The Leimbordur Mountains", "The Thaugren Forest",
    "The Pasav Desert", "The Erezetta Ocean",
    "The Isles of Schvaine", "The Cindour Islands",
    "The Library of Spires", "The Styrmwall of Acumfrial",
]

ELYSAL_AGES = [
    "Whispered Beginnings (1–500)",
    "The Miracles of Illiman (501–1000)",
    "The Era of Sail (1001–1500)",
    "An Empire of Men (1501–2000)",
    "The Rise of the North (2001–2500)",
    "A Leap of Faith (2501–3000)",
    "A Sacred Hegemony (3001–3500)",
    "The Age of Solitude (3501–4000)",
]

ELYSAL_CHARACTERS = [
    {"name": "Hipolyptica", "desc": "Founder of Hipola, unifier of the Ghenyarian tribes"},
    {"name": "Illiman", "desc": "Desert prophet whose teachings reshaped the moral world"},
    {"name": "Kratikar Klae", "desc": "Hiyalmite scholar who chronicled the Annals of Elysal"},
    {"name": "Queen Sigrid of Raxone", "desc": "Warrior-queen who forged Raxone into a military power"},
    {"name": "Emantine Asaundier", "desc": "Exile knight who liberated the Leimlands from Debrear rule"},
    {"name": "Kaed Otticus", "desc": "Finest general of the Gechann Empire, betrayed by his own state"},
    {"name": "Vittoria Medelune", "desc": "Heroine and victim of the Geckish Civil War"},
    {"name": "Zumaraya the Keeper", "desc": "Dragonborn elder who hid the children from genocide"},
    {"name": "Kael Frostmane", "desc": "Winterman clan leader who united the people of Perut"},
    {"name": "Pellae Aymundra", "desc": "Illiterate Waydrani refugee who built the Library of Spires"},
    {"name": "Ixag Taezantaenns", "desc": "Lirzan smuggler who founded the Luschnypp Syndicate"},
    {"name": "Lyssandre Elan", "desc": "Philosopher of Oradova whose lectures shook kingdoms"},
    {"name": "Tigar Pridestalker", "desc": "Pridekin warrior who earned his name against impossible odds"},
    {"name": "Iraya Devuo", "desc": "Pervin serving woman who sparked the Hyacinth Revolution"},
    {"name": "Kem Kironk", "desc": "Half-Pkoyte, half-Hiyalmite adventurer and unlikely hero"},
]
...
def verify_email(token):
    db = get_db()
    user = db.fetchone('SELECT id, username, email, joined_at FROM users WHERE verification_token = %s AND email_verified = 0', (token,))
    if not user:
...
def delete_account():
    ...
    if not user or not check_password_hash(user['password_hash'], password):
        ...
        return redirect(url_for('account_settings'))
    uid = user['id']
    db.execute('DELETE FROM reports WHERE reporter_id = %s', (uid,))
    db.execute('DELETE FROM blocks WHERE blocker_id = %s OR blocked_id = %s', (uid, uid))
